 ...
 */
 ...
 import io.cdap.wrangler.steps.AggregateStats;
 ...
 public final class SystemDirectiveRegistry implements DirectiveRegistry {
   ...
   // Additional package for AggregateStats
   private static final String WRANGLER_STEPS_PACKAGE = "io.cdap.wrangler.steps";
   ...
   public SystemDirectiveRegistry(List<String> namespaces) throws DirectiveLoadException {
     ...
     namespaces.add(WRANGLER_STEPS_PACKAGE); // Add the wrangler.steps package
     ...
     // Manually add AggregateStats if it wasn't found via reflection
     if (!registry.containsKey(AggregateStats.NAME)) {
       try {
         DirectiveInfo info = DirectiveInfo.fromSystem(AggregateStats.class);
         registry.put(info.name(), info);
       } catch (InstantiationException | IllegalAccessException e) {
         throw new DirectiveLoadException("Failed to load AggregateStats directive", e);
       }
     }
